(async function(){
  const supabaseListener = require('../card-bot/supabase-listener');
  ...
  try {
    ...
    fs.writeFileSync('card-bot/backups/test-admin-report.json', JSON.stringify({pulls: rows, distinct: distinct.length, missingCount: missing.length, missingSample: missing.slice(0,200)}, null,2));
    console.log('Report written to card-bot/backups/test-admin-report.json');
  } catch (e) {
